...
if(isset($_POST['submit'])){
   ...
   $check_cart->bind_param("i", $user_id); // Bind parameters
   $check_cart->execute();

   $check_cart_result = $check_cart->get_result(); // Get result set
   if($check_cart_result->num_rows > 0){
      ...
      if($address == ''){
         ...
         $insert_order = $con->prepare("INSERT INTO `orders`(user_id, name, number, email, method, address, total_products, total_price) VALUES(?,?,?,?,?,?,?,?)");
         $insert_order->bind_param("issssssi", $user_id, $name, $number, $email, $method, $address, $total_products, $total_price); // Bind parameters
         $insert_order->execute();

         $delete_cart = $con->prepare("DELETE FROM `cart` WHERE user_id = ?");
         $delete_cart->bind_param("i", $user_id); // Bind parameters
         $delete_cart->execute();
...
<form action="" method="post">
   ...
   <div class="cart-items">
      ...
      <?php
         ...
         $select_cart->bind_param("i", $user_id); // Bind parameters
         $select_cart->execute();
         $select_cart_result = $select_cart->get_result(); // Get result set
